...
6. Version 6 introduces a new window to the program, called the Communique Recruiter. It is used to recruit nations. It also has a built-in filter to check nations and make sure that they are from the regions which you want them to be from. There is also a filter to target your recruitment to the right part of the ideological spectrum. These are all accessible and usable using commands from `flag:recruit`, `flag:recruit -- region:x`, and `flag:recruit -- ideology:x` or using the set-up wizard.
