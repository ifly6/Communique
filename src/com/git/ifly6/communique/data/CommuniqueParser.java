...
public class CommuniqueParser {
	...
	private static boolean isTag(String token) {
		...
		} else if (token.equalsIgnoreCase("wa:delegates")) {
			return true;

		} else if (token.equalsIgnoreCase("wa:nations") || token.equalsIgnoreCase("wa:members")) {
			return true;

		} else if (token.equalsIgnoreCase("world:new")) {
			return true;
		}
	...
	private List<String> expandTag(String tag) {
		...
		if (tag.startsWith("region:")) {
			return fetcher.getRegion(tag.replace("region:", ""));

		} else if (tag.startsWith("wa:delegate")) {
			return fetcher.getDelegates();

		} else if (tag.equals("wa:nations") || tag.equals("wa:members")) {
			return fetcher.getWAMembers();

		} else if (tag.equals("world:new")) {
			return fetcher.getNew();
		}
