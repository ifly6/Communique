...
import com.git.ifly6.marconi.MarconiUtilities;
		...
		try {
			...
			for (String element : possibleRecipients) {
				...
				if (sentList.stream()   // must map to names
						...
						.anyMatch(s -> s.equalsIgnoreCase(element))) continue;
			...
			LOGGER.warning("Cannot fetch new nations [" + MarconiUtilities.currentTime() + "]. Retrying.");
